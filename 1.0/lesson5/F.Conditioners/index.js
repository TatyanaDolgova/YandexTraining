...
// Все классы школы пронумерованы последовательно от 1 до n.Известно, что для каждого класса с номером
// i, требуется ровно один кондиционер, мощность которого больше или равна ai ватт.

// Администрации школы предоставили список из m различных моделей кондиционеров, которые можно закупить.
// Для каждой модели кондиционера известна его мощность и стоимость.Требуется написать программу,
// которая определит, за какую минимальную суммарную стоимость кондиционеров можно оснастить все классы
// школы.
...
// Третья строка содержит одно целое число m(1 ≤ m ≤ 50 000) — количество предложенных моделей
// кондиционеров.
...
let classes = list[1].split(' ').map(Number);
let conditioners = list.slice(3, list.length);
let result = 0;

classes = classes.sort((a, b) => a - b);

conditioners = conditioners.sort((a, b) => +a.split(' ')[1] - +b.split(' ')[1])

let i = 0;
let j = 0;

while (i < +list[0] && j < +list[2]) {
  if (+conditioners[j].split(' ')[0] >= classes[i]) {
    result += +conditioners[j].split(' ')[1];
    i++;
  } else {
    j++;
  }
